...
def get_version():
    ...
    try:
        with open(os.path.join(ver_dir, 'VERSION')) as f:
            return f.read().strip()
...
class CreateR:
    ...
    def on_post(self, req, resp):
        ...
        if c:
            ...
            cmd += payload['filter'] + '"'
            try:
                container = c.containers.run(image='iqtlabs/ncapture:v%s' % get_version(),
...
class InfoR:
    ...
    def on_get(self, req, resp):
        resp.text = json.dumps({'version': get_version()})
        resp.content_type = falcon.MEDIA_TEXT
        resp.status = falcon.HTTP_200
        return
        ...
        try:
            for c in containers.containers.list(all=True):
                ...
                if c.attrs['Config']['Image'] == \
                        'iqtlabs/ncapture:v%s' % get_version():
                    lst = {}
