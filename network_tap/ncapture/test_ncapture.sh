...
docker build -f Dockerfile . -t cyberreboot/ncapture
echo starting ncapture
docker run --privileged --net=host --cap-add=NET_ADMIN -v $TMPDIR:/files -t cyberreboot/ncapture /tmp/run.sh $URI 15 test 1 "host $IP and icmp" -d 12 -s 4 -a none -c none -o /files/ || exit 1 &
